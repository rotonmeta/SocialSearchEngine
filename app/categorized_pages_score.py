...
def similarity_categorized_pages(A, B, cat, coeff):
    tmp = []
    tmp_A = []
    for a in A:
        if a[cat] not in tmp:
...
def update_vector(vector, cat, found_list):
    ...
    for a in vector:
        if a[cat] not in found_list:
            if '_null_' not in str(a['genre']):
                A.append({'name': a['name'], 'genre': a['genre'], 'fan_count': 1})
    return A


def categorized_pages_score(solr, userA_id, userB_id, category):
    pagesA = solr.search(q='doc_type:page AND user_id:' + str(userA_id) + ' AND type:' + str(category), rows=pow(10, 6),
                         wt='python')
    pagesB = solr.search(q='doc_type:page AND user_id:' + str(userB_id) + ' AND type:' + str(category), rows=pow(10, 6),
                         wt='python')
    tmp_A = []
    tmp_B = []
    ...
    cat = 'page_id'
    coeff1 = [1.0]
    similarity = similarity_categorized_pages(tmp_A, tmp_B, cat, coeff1)
    ...
    cat = 'genre'
    coeff2 = [0.2]
    similarity = similarity_categorized_pages(tmp_A, tmp_B, cat, coeff2)
...
def update_categorized_pages_score(solr, usersID_list, cat):
    docs_list = []
    for i in range(0, len(usersID_list)):
        for j in range(i + 1, len(usersID_list)):
            ...
            try:
                solr.delete(q="type:" + str(cat) + "_pages_score AND (" + query + ")")
            except:
                print('primo aggiornamento')
            score = categorized_pages_score(solr, a, b, cat)
            new_doc = {"doc_type": 'score', 'type': str(cat) + "_pages_score", "userA": a, "userB": b, "score": score}
